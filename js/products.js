...
async function openProductModal(productId) {
  ...
  try {
    ...
    // Quantity input starts empty so users can freely type. Clamping only
    // runs when the value EXCEEDS available stock; we never auto-fill a "1"
    // back into an empty field (that caused the "13 → 1" override bug when
    // the user typed "3" with a prefilled "1" still in place).
...
async function addToCartFromModal(productId) {
  ...
  const isFca = !!document.getElementById('fca-file-select');
  const folderSel = document.querySelector('#fca-folder-select .custom-select-display');
  const sel = isFca ? folderSel : document.querySelector('#options-select .custom-select-display');
  const optionId = sel?.dataset.value;
  const stockFileId = isFca
    ? document.querySelector('#fca-file-select .custom-select-display')?.dataset.value || null
    : null;
  const qty = parseInt(document.getElementById('totalQuantity').value) || 1;
  ...
  if (isFca) {
    const { stock, fileId } = fcaGetSelection();
    if (!fileId) { showToast('Please select a stock file', 'error'); return; }
    if (stock <= 0) { showToast('This file is out of stock', 'error'); return; }
    if (qty > stock) { showToast('Quantity exceeds available stock', 'error'); return; }
  }
  ...
  try {
    const body = { productId, optionId, quantity: qty };
