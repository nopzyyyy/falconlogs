(() => {
  ...
  // Telegram Login Interactivity
  const tgLoginBtn = document.querySelector("#tgLoginBtn");
  const tgLoginModalOverlay = document.querySelector("#tgLoginModalOverlay");
  const closeTgLoginModal = document.querySelector("#closeTgLoginModal");
  const tgTokenInput = document.querySelector("#tgTokenInput");
  // Telegram Login Interactivity
  const tgTokenGroup = document.querySelector("#tgTokenGroup");
  const tgOtpGroup = document.querySelector("#tgOtpGroup");
  const tgOtpInput = document.querySelector("#tgOtpInput");
  const tgBackToToken = document.querySelector("#tgBackToToken");
  const tgSubmitText = document.querySelector("#tgSubmitText");

  let authStep = "token"; // "token" or "otp"

  function showTokenStep() {
    authStep = "token";
    if (tgTokenGroup) tgTokenGroup.style.display = "block";
    if (tgOtpGroup) tgOtpGroup.style.display = "none";
    if (tgSubmitText) tgSubmitText.textContent = "Verify and Log in";
    if (tgTokenInput) tgTokenInput.disabled = false;
    tgModalError.hidden = true;
  }

  function showOtpStep() {
    authStep = "otp";
    if (tgTokenGroup) tgTokenGroup.style.display = "none";
    if (tgOtpGroup) tgOtpGroup.style.display = "block";
    if (tgSubmitText) tgSubmitText.textContent = "Confirm OTP";
    if (tgOtpInput) {
      tgOtpInput.value = "";
      tgOtpInput.disabled = false;
      tgOtpInput.focus();
    }
    tgModalError.hidden = true;
  }

  if (tgLoginBtn && tgLoginModalOverlay) {
    tgLoginBtn.addEventListener("click", () => {
      tgModalError.hidden = true;
      if (tgTokenInput) tgTokenInput.value = "";
      showTokenStep();
      tgLoginModalOverlay.classList.add("active");
    });
  }

  if (closeTgLoginModal && tgLoginModalOverlay) {
    closeTgLoginModal.addEventListener("click", () => {
      tgLoginModalOverlay.classList.remove("active");
    });
  }

  if (tgBackToToken) {
    tgBackToToken.addEventListener("click", (e) => {
      e.preventDefault();
      showTokenStep();
    });
  }
  ...
  if (tgSubmitBtn) {
    tgSubmitBtn.addEventListener("click", async () => {
      const token = tgTokenInput.value.trim().toUpperCase();
      if (!token) {
        tgModalError.textContent = "Please enter your Login Token.";
        tgModalError.hidden = false;
        tgTokenInput.focus();
        return;
      }
      if (!token.startsWith("TG-")) {
        tgModalError.textContent = "Invalid token format. Must start with 'TG-'.";
        tgModalError.hidden = false;
        tgTokenInput.focus();
        return;
      }
      ...
      if (authStep === "token") {
        // Step 1: Request OTP
        tgModalError.hidden = true;
        tgSubmitBtn.disabled = true;
        const originalBtnHtml = tgSubmitBtn.innerHTML;
        tgSubmitBtn.innerHTML = `<span class="spinner-sm"></span> Sending OTP...`;
        ...
        try {
          ...
          tgSubmitBtn.disabled = false;
          tgSubmitBtn.innerHTML = originalBtnHtml;

          if (!res.ok) {
            tgModalError.textContent = body.error || "Token verification failed.";
            tgModalError.hidden = false;
            return;
          }
          ...
          tgModalError.textContent = "Network error. Please try again.";
          tgModalError.hidden = false;
          tgSubmitBtn.disabled = false;
          tgSubmitBtn.innerHTML = originalBtnHtml;
        }
        ...
        // Step 2: Verify OTP
        const otp = tgOtpInput.value.trim();
        if (!otp || otp.length !== 6 || !/^\d+$/.test(otp)) {
          tgModalError.textContent = "Please enter a valid 6-digit OTP code.";
          tgModalError.hidden = false;
          tgOtpInput.focus();
          return;
        }
        ...
        tgSubmitBtn.disabled = true;
        const originalBtnHtml = tgSubmitBtn.innerHTML;
        tgSubmitBtn.innerHTML = `<span class="spinner-sm"></span> Verifying OTP...`;
        ...
        try {
          ...
          if (!res.ok) {
            tgModalError.textContent = body.error || "OTP verification failed.";
            tgModalError.hidden = false;
            return;
          }
